...
static uint32_t periodic_task(void)
{
    if(button_event)
    {
        button_event = 0;
...
static void on_prov_proxy_enabled_cb(bool enabled)
{
    if (enabled)
    {
        App_Scheduler_addTask_execTime(periodic_task, APP_SCHEDULER_SCHEDULE_ASAP, 10);
    }
    else
    {
        App_Scheduler_cancelTask(periodic_task);
...
void App_init(const app_global_functions_t * functions)
{
    ...
    app_lib_settings_role_e role;
    lib_settings->getNodeRole(&role);
    local_provisioning_config_t conf = {
        .timeout_s = (role & APP_LIB_SETTINGS_LL_ROLE_BIT) ? 20 : 120
    };
    Local_provisioning_init(&m_psk, on_prov_proxy_enabled_cb, &conf);
