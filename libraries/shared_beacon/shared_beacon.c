                                              ...
                                              uint8_t * shared_beacon_index)
{
    ...
    *shared_beacon_index = n;
    ...
    if (!m_beacons_enabled)
    {
        ...
        LOG(LVL_INFO, "lib_beacon_tx->enableBeacons");
    }
...
/**
 * @brief   Returns the count of active beacons.
 * @return  count of active beacons
 */
static uint8_t count_active_beacons(void)
{
    uint8_t n;
    uint8_t count = 0;
...
shared_beacon_res_e Shared_Beacon_stopBeacon(uint8_t shared_beacon_index)
{
    ...
    if(count_active_beacons() == 0) {
        lib_beacon_tx->clearBeacons();
        m_beacons_enabled = false;
        LOG(LVL_INFO, "lib_beacon_tx->clearBeacons");
    }
