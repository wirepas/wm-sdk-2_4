...
static uint32_t schedule_retry(uint8_t attempt)
{
    ...
    LOG(LVL_INFO,"Scheduling retry #%u in %lums", attempt, delay_ms);
...
static uint32_t state_idle(void)
{
    ...
    if (m_events.start)
    {
        ...
        LOG(LVL_INFO, "State START : delayed %d s",new_delay_ms);
    }
...
static uint32_t state_start(void)
{
    ...
    {
        if (App_Scheduler_addTask_execTime(timeout_task, DELAY_WAIT_END_SCAN_MS + JOINING_RX_TIMEOUT, 500) !=
    ...
    if (m_events.scan_end)
    {
        ...
        if (m_num_beacons > 0)
        {
            ...
            if(beacon != NULL)
            {
                ...
                {
                    address_check_ok = (res_param && (addr == beacon->addr));
                }
                else
                {
