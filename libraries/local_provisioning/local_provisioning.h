...
/**
 * \brief   Configuration for the Local provisioning library
 */
typedef struct
{
    /** Provisioning session timeout in seconds */
    uint16_t timeout_s;
 ...
 *          key.
 * \param   on_proxy_enabled_cb
 *          Callback notified when proxy is enabled or disabled. May be NULL.
 * \param   conf
 *          Provisioning configuration. If NULL, defaults are used.
