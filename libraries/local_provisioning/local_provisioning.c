...
static local_provisioning_config_t m_config = { .timeout_s = 20 };
...
static bool start_cb(const uint8_t *                  p_uid,
                     uint8_t                          uid_len,
                     ...
                     provisioning_proxy_net_param_t * p_param)
{
    ...
    /* Log Extended UID information if present */
    if (PROV_METHOD_IS_EXTENDED_UID(method))
    {
        if (prov_uid != NULL && uid_len > 0)
        {
            LOG(LVL_INFO, "Extended UID provisioning request:");
        ...
        /* Add any Extended UID validation here if needed */
        /* For now, accept all Extended UID requests */
    }
    else
    {
        LOG(LVL_INFO, "Standard provisioning request (method: %d)", method);
        if (prov_uid != NULL && uid_len > 0)
        {
    ...
    res = lib_settings->getEncryptionKey(p_param->enc_key);
    if (res != APP_RES_OK)
    {
        LOG(LVL_ERROR, "Cannot get encryption key %d", res);
        return false;
    }

    res = lib_settings->getAuthenticationKey(p_param->auth_key);
    if (res != APP_RES_OK)
    {
        LOG(LVL_ERROR, "Cannot get authentication key %d", res);
        return false;
    }

    res = lib_settings->getNetworkAddress(&p_param->net_addr);
    if (res != APP_RES_OK)
    {
        LOG(LVL_ERROR, "Cannot get network address %d", res);
        return false;
    }

    res = lib_settings->getNetworkChannel(&p_param->net_chan);
    if (res != APP_RES_OK)
    {
        LOG(LVL_ERROR, "Cannot get network channel %d", res);
        return false;
    }
...
Local_provisioning_res_e Local_provisioning_init(local_provisioning_psk_t * psk,
                                                 ...
                                                 const local_provisioning_config_t * conf)
{
    m_reset_node = false;
    if (psk == NULL)
    {
                                                          ...
                                                          local_provisioning_joining_done_cb cb_end)
{
    ...
    {
        .settings.nb_retry = 3,
        .settings.timeout_s = m_config.timeout_s,
        .settings.method = PROV_METHOD_SECURED, /* Always use secure method with default key if keys not set */
        .settings.prov_uid_len = sizeof(uid),
        .end_cb = end_cb,
        // No user data cb received from local provisioning
        .user_data_cb = NULL,
        .beacon_joining_cb = joining_beacon_selection
    };
    // Copy UID and keys to provisioning settings
    memcpy(conf.settings.prov_uid.uid, &uid, sizeof(uid));
    memcpy(&conf.settings.prov_keys.auth_key,
                    &m_psk_init.psk[0], PROV_KEY_SIZE_BYTES);
    memcpy(&conf.settings.prov_keys.enc_key,
                    &m_psk_init.psk[PROV_KEY_SIZE_BYTES], PROV_KEY_SIZE_BYTES);
    if (cb_beacons != NULL)
    {
    ...
    // Start provisioning in 20 seconds as starting the stack will
    // trigger a normal network scan by which causes a conflict
    // with the joining network scan
    if (App_Scheduler_addTask_execTime(provisioning_start_task, 20 * 1000, 50) != APP_SCHEDULER_RES_OK)
    {
        return LOCAL_PROVISIONING_RES_INTERNAL_ERROR;
    }
    return LOCAL_PROVISIONING_RES_SUCCESS;
